function [DCM,dipfit] = mmn_spm_dcm_erp_MP_eP(DCM)
% Edit US 20230129: function renamed
% prev: mmn_spm_dcm_erp
% new:  mmn_spm_dcm_erp_MP_eP
...
%--------------------------------------------------------------------------
[pE,pC] = mmn_spm_dcm_neural_priors(DCM.A,DCM.B,DCM.C,model);
...
%--------------------------------------------------------------------------
[x,f,h] = mmn_spm_dcm_x_neural(pE,model);
...
% Edit US 20230108: define separate starting values for EM: inversion
% Reason: iEEG derived posteriors seem to result in "local min solution"

% Load standard priors, adjust dimensions and set as starting values

[pE_standard,pC_standard]  = ...
    spm_dcm_neural_priors(DCM.A,DCM.B,DCM.C,DCM.options.model);

% Use non-standard priors as EM starting value
pE_standard.G = M.pE.G;

M.P    = pE_standard;

% Check function: spm_nlsi_N (line 262) for details (line 185)
...
if strcmp(M.dipfit.type,'IMG') && DATA
    ...
    % Edit US 20230113: not saving individual inversions
    % Reason: each inversion is about 10MB big
...
% Edit US 20230113: not saving individual DCMs
% Save "group DCM" GCM instead > main function
