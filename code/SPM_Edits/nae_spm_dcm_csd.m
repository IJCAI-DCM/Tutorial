function DCM = nae_spm_dcm_csd(DCM)
% RR change: point to mni-specific feature selection (log scaled CSD)
...
%--------------------------------------------------------------------------
[pE,pC]  = nae_spm_dcm_neural_priors(DCM.A,DCM.B,DCM.C,model);              % RR Edit call neural priors function
...
%--------------------------------------------------------------------------
[x,f]    = nae_spm_dcm_x_neural(pE,model);
 
% create DCM
%--------------------------------------------------------------------------
DCM.M.IS = 'spm_csd_mtf';
DCM.M.FS = 'nae_spm_fs_csd';    % RR Change
DCM.M.g  = 'spm_gx_erp';
DCM.M.f  = f;
DCM.M.x  = x;
...
%--------------------------------------------------------------------------
y     = nae_spm_fs_csd(DCM.xY.y,DCM.M);
for i = 1:length(y)
    m      = spm_length(y{i});
    Q{i,i} = speye(m,m);
end
